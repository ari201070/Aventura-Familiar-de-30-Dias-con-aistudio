  ...
  return (
    <div className="space-y-12">
              ...
              {TRANSPORT_DATA.map((leg: TransportLeg) => {
                ...
                return (
                  <tr key={leg.id} className="hover:bg-gray-50 transition-colors">
                    <td className={`px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900 ${textAlignClass}`}>{t(leg.fromKey)}</td>
                    <td className={`px-4 py-3 whitespace-nowrap text-sm text-gray-500 ${textAlignClass}`}>{t(leg.toKey)}</td>
                    <td className={`px-4 py-3 whitespace-nowrap text-sm text-gray-500 ${textAlignClass}`}>{t(leg.meanKey)}</td>
                    <td className={`px-4 py-3 whitespace-nowrap text-sm text-gray-500 ${textAlignClass}`}>{t(leg.timeKey)}</td>
                    <td className={`px-4 py-3 whitespace-nowrap text-sm text-gray-500 ${textAlignClass}`}>{getConvertedPrice(leg.basePriceARS)}</td>
                    <td className={`px-4 py-3 whitespace-nowrap text-sm text-gray-500 ${textAlignClass}`} dangerouslySetInnerHTML={{ __html: leg.company }}></td>
                  </tr>
                );
              })}
            </tbody>
      ...
      <section className={cardClasses}>
        ...
        <div className="mb-8 p-4 bg-indigo-50 rounded-lg shadow-inner">
            ...
            <ul className="list-disc list-inside space-y-2 pl-5">
                {CITIES.map(city => {
                    const durationTextKey = `${city.id}_dates_duration`;
                    const durationText = t(durationTextKey);
                    let simplifiedDuration = t('duration_not_specified');

                    if (durationText && durationText !== durationTextKey) {
                        const firstLine = durationText.split('\n')[0];
                        // Remove common prefixes and markdown for "Estadía"
                        simplifiedDuration = firstLine.replace(/^- \*\*Estadía\*\*:\s*|- Estadía:\s*|\*\*Estadía\*\*:\s*|Estadía:\s*/i, '').trim();
                        if (simplifiedDuration === firstLine.trim()) { // If no prefix was removed, it might be just the duration or something else
                           // Try a more generic removal of list markers or bolding if it's just a simple duration
                           simplifiedDuration = simplifiedDuration.replace(/^- \s*|\*\*\s*(.*?)\s*\*\*/g, '$1').trim();
                        }
                    }
                    
                    return (
                        <li key={city.id} className="text-gray-700">
                            <span className="font-medium text-gray-800">{t(city.nameKey)}:</span> {simplifiedDuration}
                        </li>
                    );
                })}
            </ul>
        ...
        <div className="p-4 bg-purple-50 rounded-lg shadow-inner">
            <h3 className="text-xl font-semibold text-purple-700 mb-3">{t('itinerary_program_optimization_tips_title')}</h3>
            <ul className="list-disc list-inside space-y-2 pl-5">
      ...
      <section className={cardClasses}>
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6 items-center">
          ...
          <select
            ...
            aria-label={t('packing_essential') + '/' + t('packing_optional')}
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <ul className="space-y-2" aria-labelledby="packing_essential_heading">
              {packingItems.filter(item => item.type === 'essential').map(item => (
                <li key={item.id} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg shadow-sm">
                  ...
                  <button onClick={() => handleRemovePackingItem(item.id)} className="text-red-500 hover:text-red-700" aria-label={`${t('Remove')} ${item.text}`}>
          ...
          <div>
            ...
            <ul className="space-y-2" aria-labelledby="packing_optional_heading">
      ...
      <section className={cardClasses}>
        ...
        {aiResponse && (
          <div className="mt-6 p-4 bg-gray-50 rounded-lg shadow whitespace-pre-wrap text-gray-700" role="alert">
            {aiResponse}
          </div>
        )}
      </section>
      ...
      <section className={cardClasses}>
        ...
        <form onSubmit={handleCurrencyConversion} className="space-y-4 sm:space-y-0 sm:flex sm:items-end sm:gap-4">
          ...
          <div className="self-end pb-3 text-2xl text-gray-500 hidden sm:block" aria-hidden="true">→</div>
          <div>
        ...
        {conversionResult && (
          <div className="mt-6 p-4 bg-gray-50 rounded-lg shadow text-center font-semibold text-lg text-gray-700" role="status">
            {conversionResult}
          </div>
        )}
      </section>
