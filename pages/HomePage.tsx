              ...
              {TRANSPORT_DATA.map((leg: TransportLeg) => {
                return (
                  <tr key={leg.id} className="hover:bg-gray-50 transition-colors">
                    <td className={`px-4 py-3 whitespace-nowrap text-sm font-semibold text-gray-800 ${language === 'he' ? 'text-right' : 'text-left'}`}>{t(leg.fromKey)}</td>
                    <td className={`px-4 py-3 whitespace-nowrap text-sm font-semibold text-gray-800 ${language === 'he' ? 'text-right' : 'text-left'}`}>{t(leg.toKey)}</td>
                    <td className={`px-4 py-3 whitespace-nowrap text-sm text-gray-600 ${language === 'he' ? 'text-right' : 'text-left'}`}>{t(leg.meanKey)}</td>
                    <td className={`px-4 py-3 whitespace-nowrap text-sm text-gray-600 ${language === 'he' ? 'text-right' : 'text-left'}`}>{t(leg.timeKey)}</td>
                    <td className={`px-4 py-3 whitespace-nowrap text-sm text-gray-600 ${language === 'he' ? 'text-right' : 'text-left'}`}>{getConvertedPrice(leg.basePriceARS)}</td>
                    <td className={`px-4 py-3 whitespace-nowrap text-sm text-gray-600 ${language === 'he' ? 'text-right' : 'text-left'}`} dangerouslySetInnerHTML={{ __html: leg.company }}></td>
                  </tr>
                );
              })}
            </tbody>
